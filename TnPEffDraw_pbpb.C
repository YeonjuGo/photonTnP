...
void TnPEffDraw_pbpb(TString inputPrefix="passISO_VoigtErfExp_recoIDPho_v29_isoEff_diPhoMass_mass60to110_pt15to200_binsForFit30_isoEff",TString inputTag="recoIDPho_pt", TString st_dep = "ptDep", bool isCntEff=false) {
    ...
    const char* fDataName = Form("%sNew_tnp_Ana_data_pbpb_%s.root",path.Data(),inputPrefix.Data());
    const char* fMCName =  Form("%sNew_tnp_Ana_mc_pbpb_%s.root",path.Data(),inputPrefix.Data());
    TString fout_name = Form("correction_functions_pbpb_%s_%s_%s.txt",st_dep.Data(), inputPrefix.Data(),inputTag.Data());
    if(isCntEff) fout_name = Form("correction_functions_pbpb_%s_%s_%s_cntEff.txt",st_dep.Data(), inputPrefix.Data(),inputTag.Data());
    ...
    RooDataSet *daData0 = (RooDataSet*)f9->Get(cutTag + "/" + inputTag + "/" + st_efftype); 
    RooDataSet *daData1 = (RooDataSet*)f10->Get(cutTag + "/" + inputTag + "/" + st_efftype); 
    cout << "Plot efficiency TGraphAsymmErrors"<<endl;
    TString var = "probe_pt"; 
    if(st_dep=="ptDep") var = "probe_pt";
    else if(st_dep=="centDep") var = "centBin"; 
    else cout << "[Error] : No this kind of variables !!!";
    TGraphAsymmErrors *ComPt0 = plotEff_1bin(daData0, 1, Form("%s",var.Data()));
    cout << "DATA DONE"<<endl;
    TGraphAsymmErrors *ComPt1 = plotEff_1bin(daData1, 1, var);
    //TGraphAsymmErrors *ComPt1 = plotEff_1bin(daData1, 1, Form("%s",var.Data()));
    cout << "MC DONE"<<endl;

    cout << "Set graph cosmetics "<<endl;
    ...
    if(st_dep=="ptDep"){ 
        ...
        xMin = 20; 
        xMax = 200; 
        //xMin = ((RooRealVar*) daData0->get()->find("probe_pt"))->getBinning().binLow(0);
        //xMax = ((RooRealVar*) daData0->get()->find("probe_pt"))->getBinning().binHigh(0);
        cout << "pT ranges : ["<< xMin << "," << xMax << "]"<< endl;
    }
    else if(st_dep=="centDep"){ 
        xTitle = "Centrality (hiBin)";
        xMin = 0; 
        xMax = 200; 
        //xMin = ((RooRealVar*) daData0->get()->find("centBin"))->getBinning().binLow(0);
        //xMax = ((RooRealVar*) daData0->get()->find("centBin"))->getBinning().binHigh(0);
        cout << "centrality ranges : ["<< xMin << "," << xMax << "]"<< endl;
    } else{

        cout << "[ERROR] !!!! " << endl;
    }
    TH1F *hPad = new TH1F("hPad",Form(";E_{T}^{#gamma} [GeV];%s Efficiency",""),5,20,200);
    TH1F *hPad1 = new TH1F("hPad1",Form(";%s;Efficiency",xTitle.Data()),10,xMin,xMax);
    ...
    cout <<"Draw graphs in the panel"<<endl;
    pad1->cd();
    hPad1->Draw();
    ...
    if(st_dep=="ptDep"){ 
        double centMin = ((RooRealVar*) daData0->get()->find("centBin"))->getBinning().binLow(0);
        double centMax = ((RooRealVar*) daData0->get()->find("centBin"))->getBinning().binHigh(0);
        //leg2->SetHeader(TString("#splitline{") + Form(" Efficiency}{(centrality %d-%d %s)}",(int)centMin/2,(int)centMax/2,"%"));
    } else if(st_dep=="centDep"){ 
        double ptmin = ((RooRealVar*) daData0->get()->find("probe_pt"))->getBinning().binLow(0);
        //double ptmax = ((RooRealVar*) daCentData0->get()->find("probe_pt"))->getBinning().binHigh(0);
        //leg1->SetHeader(TString("#splitline{") + Form(" Efficiency}{(%.1f<E_{T}<%.1f GeV)}",ptmin,ptmax));
        //leg2->SetHeader(TString("#splitline{") + Form(" Efficiency}{(E_{T}>%.1f GeV)}",ptmin));
    }
    sprintf(legs2,"MC");
    ...
    leg2->Draw("same");
    leg2->Draw("same");
    //drawText("Centrality 0-100 %", xpos, ypos-2*dy); 
    //drawText("Centrality 0-100 %", xpos, ypos-3*dy); 
